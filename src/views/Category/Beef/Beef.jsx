...
const Beef = (props) => {
    ...
    useEffect(() => {
      let fetchData = async () => {
        ...
        let response = await fetch("https://demo1273150.mockable.io/poultry");
    ...
    if (fetchedData.length) {
                ...
                <ul>
                    {  
                        fetchedData.map((dish, index) => (
                            <li key={index}>
                                <Link to={`/menu/beef/${dish.slug}`}>pic{/* <img src={dish.image._url}/> */}</Link>  
                                <div>{dish.name}</div>
                            </li>
                        ))
                    }
      ...
      displayContent = <div>Beef is coming soon</div>;
    }
