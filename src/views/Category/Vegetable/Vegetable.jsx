...
const Vegetable = (props) => {
    ...
    useEffect(() => {
      let fetchData = async () => {
        ...
        let response = await fetch("https://demo1273150.mockable.io/vegetable");
    ...
    if (fetchedData.length) {
                ...
                <ul>
                    {  
                        fetchedData.map((dish, index) => (
                            <li key={index}>
                                <Link to={`/menu/vegetable/${dish.slug}`}>pic{/* <img src={dish.image._url}/> */}</Link>  
                                <div>{dish.name}</div>
                            </li>
                        ))
                    }
      ...
      displayContent = <div>Vegetable is coming soon</div>;
    }
