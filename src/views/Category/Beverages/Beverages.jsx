...
const Beverages = (props) => {
    ...
    useEffect(() => {
      let fetchData = async () => {
        ...
        let response = await fetch("https://demo1273150.mockable.io/beverages");
    ...
    if (fetchedData.length) {
                ...
                <ul>
                    {  
                        fetchedData.map((dish, index) => (
                            <li key={index}>
                                <Link to={`/menu/beverages/${dish.slug}`}>pic{/* <img src={dish.image._url}/> */}</Link>  
                                <div>{dish.name}</div>
                            </li>
                        ))
                    }
      ...
      displayContent = <div>Beverages are coming soon</div>;
    }
